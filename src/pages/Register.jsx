...
export default function Register() {
  ...
  const handleGoogleSignUp = async () => {
    ...
    if (error) {
      setError(error.message || 'فشل التسجيل عبر جوجل');
      setLoading(false);
    }
  };
  ...
  const handleAppleSignUp = async () => {
    ...
    if (error) {
      setError(error.message || 'فشل التسجيل عبر Apple');
      setLoading(false);
    }
  };
  ...
  if (success) {
    return (
      <div
        className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 via-blue-50 to-purple-50 p-4"
        dir="rtl"
      >
        <Card className="w-full max-w-md shadow-xl">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-gray-600 mb-4">
              مرحباً بك في إدارة البيت الذكي
            </p>
