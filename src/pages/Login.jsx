...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (result?.error) {
      setError(result.error.message || 'حدث خطأ أثناء تسجيل الدخول');
      setLoading(false);
    } else {
      navigate('/');
    }
  ...
  const handleGoogleLogin = async () => {
    ...
    if (result?.error) {
      setError(result.error.message || 'فشل تسجيل الدخول عبر جوجل');
      setLoading(false);
    }
  };
  ...
  const handleAppleLogin = async () => {
    ...
    if (error) {
      setError(error.message || 'فشل تسجيل الدخول عبر Apple');
      setLoading(false);
    }
  };
      ...
      <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md">
        ...
        <div className="flex gap-3">
          <button
            ...
            disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 bg-white border border-gray-300 text-gray-700 py-3 rounded-lg font-medium hover:bg-gray-50 transition-all disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={loading}
            className="flex-1 flex items-center justify-center gap-2 bg-black text-white py-3 rounded-lg font-medium hover:bg-gray-900 transition-all disabled:opacity-50"
          >
